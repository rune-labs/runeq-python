...
class StreamMetadata(ItemBase):
    ...
    def get_stream_dataframe(
        ...
        Args:
            ...
            graph_client: If specified, this client is used to fetch metadata
                from the API. Otherwise, the global GraphClient is used.
    ...
    def get_stream_availability_dataframe(
        ...
        and metadata. batch_operation must be specified if len(stream_ids) > 1.
...
def get_stream_dataframe(
    ...
    if len(stream_metadata) > 1:
        raise RuneError(
            f"Expected 1 stream, but got {len(stream_metadata)}."
        )

    # Remove some columns to reduce verbosity
    metadata = stream_metadata.get(stream_id).to_dict()
    metadata['stream_type_id'] = metadata['stream_type']['id']
    metadata['stream_id'] = metadata['id']
    del metadata['id']
    del metadata['stream_type']
